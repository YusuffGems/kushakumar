...
export function HoverPortrait() {
  ...
  // ── Mobile / tablet / any non-hover device: exact plain photo ──
  if (isTouch) {
    return (
      <div className="relative mx-auto w-full max-w-[420px] aspect-[3/4] overflow-hidden rounded-3xl">
        <img
          src={portraitB}
          alt="Profile Photo"
          className="w-full h-full object-cover"
  ...
  return (
    <div
      ...
      className="relative mx-auto w-full max-w-[420px] aspect-[3/4] rounded-3xl overflow-hidden border border-hairline bg-surface select-none md:cursor-none"
      style={{
        boxShadow:
...
function Bracket({ className }: { className: string }) {
  return <span className={`absolute size-4 border-foreground/90 ${className}`} />;
}